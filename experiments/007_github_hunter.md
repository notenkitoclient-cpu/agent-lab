---
id: 007
agent: github-hunter
title: GitHub Trending: opendataloader-pdf
date: 2026-03-20
status: complete
---

## Discovered Repository

| Property | Value |
|----------|-------|
| Repository | [opendataloader-project/opendataloader-pdf](https://github.com/opendataloader-project/opendataloader-pdf) |
| Description | PDF Parser for AI-ready data. Automate PDF accessibility. Open-source. |
| Stars | 6465 |
| Forks | 467 |
| Language | Java |
| Topics | a11y, accessibility, ai, bounding-box, document-parsing, eaa, html, json, markdown, ocr, ocr-recognition, pdf, pdf-accessibility, pdf-converter, pdf-extraction, pdf-parser, pdf-ua, rag, tables, tagged-pdf |

---

## AI Summary

（要約エラー: Error code: 400 - {'type': 'error', 'error': {'type': 'invalid_request_error', 'message': 'Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.'}, 'request_id': 'req_011CZE9no4ihm1B5QAG8dpEv'}）
説明: PDF Parser for AI-ready data. Automate PDF accessibility. Open-source.

---

## X Post Draft

```text
Agent Experiment #007

Discovered an interesting AI tool 👀

Tool: opendataloader-pdf

What it does:
（要約エラー: Error code: 400 - {'type': 'error', 'error': {'type': 'invalid_request_error', 'message': 'Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.'}, 'request_id': 'req_011CZE9no4ihm1B5QAG8dpEv'}）
説明: PDF Parser for AI-ready data. Automate PDF accessibility. Open-source.

⭐ 6465 stars
🔗 Repo: https://github.com/opendataloader-project/opendataloader-pdf

#AgentLab #AIAgent #GitHub
```

---

*Agent Lab - Build. Experiment. Automate.*
